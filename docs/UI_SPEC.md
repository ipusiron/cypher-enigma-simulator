...
- ドラッグハンドル（☰）でドラッグ&ドロップ順序変更可能
  - 並び替えても元のラベル番号（Scrambler 1, 2, 3）は維持される
  - 処理順序は画面上の左から右の順序に従う
- ON/OFFトグル
  - Scrambler 1: デフォルトON
...
- ノッチ設定
  - ON/OFFトグル（デフォルト: ON）
  - 位置入力（A-Z、デフォルト: Z）
  - ONの場合: このスクランブラーが設定位置を超えると**次のスクランブラー**が1回転
  - OFFの場合: 26回で次段が1回転（従来動作）
  - 注意: 最後のスクランブラーのノッチは効果がない（後続がないため）
...
- 表示内容:
  - 配線設定（配線一覧）
  - 各スクランブラーのノッチ状態（ON時は位置、OFF時は「OFF」）
  - 各文字の処理過程
  - 入出力矢印（↓↑）による視覚化
  - スクランブラーのシフト状態
  - スクランブラーラベル（元のID番号を使用）
- コピーボタン付き
...
- スクランブラー 1-3
  - ON/OFFトグル
  - プリセット選択＋配線入力
  - ノッチ設定（ON/OFFトグル＋位置入力）
  - ドラッグ&ドロップで順序変更可能（ラベル番号は維持）
